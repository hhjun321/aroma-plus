...
name: feature-dev
description: >
  MANDATORY: Use this skill for ALL implementation requests without exception — new features,
  existing feature improvements, bug fixes, refactoring, UI changes, configuration changes,
  or any other code modification. Do NOT implement code directly before running this pipeline.
  Trigger phrases include but are not limited to: "진행시켜", "구현해줘", "수정해줘", "추가해줘",
  "변경해줘", "고쳐줘", "만들어줘", "적용해줘", "해줘", "proceed", "implement", "fix", "add", "change".
---

# 구현 파이프라인 (Implementation Pipeline)

**모든 구현 건에 예외 없이 적용되는 파이프라인입니다.**
신규 기능, 기존 기능 보완, 버그 수정, 리팩토링, UI 변경 등 코드를 수정하는 모든 작업이 이 과정을 거칩니다.
...
각 단계별로 사용할 모델이 지정되어 있습니다. Agent 툴 호출 시 `model` 파라미터를 반드시 명시합니다.

| 단계 | 에이전트 | 모델 | 이유 |
|------|---------|------|------|
| STEP 1 | `planner` | **opus** | 전체 영향 범위·위험 요소를 깊이 추론해야 함 |
| STEP 2 | `code-explorer` | **haiku** | 파일 탐색·패턴 검색 위주 — 고성능 불필요, 비용 절감 |
| STEP 4 | `code-architect` | **opus** | 설계 트레이드오프를 다각도로 판단해야 함 |
| STEP 6 | `code-reviewer`, 언어별 리뷰어 | **sonnet** (기본값) | 코딩 품질 체크는 Sonnet이 최적, reasoning보다 코딩 능력이 중요 |
| STEP 7 | `security-reviewer` | **opus** | 놓쳤을 때 비용이 크고, 전체 코드 패턴을 꿰뚫어야 함 |

---

## STEP 1 — 계획 수립 (`planner` 에이전트, model: **opus**)

**반드시 `planner` 서브에이전트를 즉시 호출합니다. `model: "opus"`를 명시합니다.**

planner 에이전트에게 다음을 포함한 구현 계획 수립을 위임합니다:
- **작업 요약**: 무엇을 만들거나 수정하는지 (신규/보완/수정/리팩토링 여부 포함)
- **영향 범위**: 수정/추가할 파일 목록
- **데이터 모델 변경 여부**: DB 마이그레이션, 스키마 변경 등 (필요하다면 프로젝트에서 지원하는 DB 엔진 전체 대상)
- **API 엔드포인트**: 추가되거나 변경되는 라우트와 HTTP 메서드 (해당 시)
- **인증/권한 설계**: 접근 제어가 필요한 경우 (해당 시)
- **작업 순서**: 의존성을 고려한 구현 순서
- **위험 요소**: 기존 코드에 영향을 줄 수 있는 부분 (특히 기존 기능 보완 시 회귀 가능성)
- **역방향 영향도**: 이 기능이 생성·변경·파괴하는 상태(데이터/불변식)를 전제로 동작하는 기존 로직을 열거. delete/remove/revoke/bulk 계열이면 "없음(0개)" 상태가 만들어졌을 때 기존 코드가 어떻게 반응하는지 반드시 명시
- **예상 트래픽 및 데이터 규모**: 대용량 데이터 처리나 높은 동시성이 예상되는지 파악

planner 결과를 요약하여 보여준 후 **사용자의 진행 승인을 받습니다.**

---

## STEP 2 — 코드베이스 탐색 (2-3개 `code-explorer` 에이전트 병렬 실행, model: **haiku**)

**반드시 2-3개의 `code-explorer` 서브에이전트를 병렬로 호출합니다. `model: "haiku"`를 명시합니다.**

각 에이전트는 서로 다른 각도에서 탐색합니다:
- 수정 대상 코드의 현재 구현 상태 및 의존 관계 분석
- 유사 기능 구현 사례 추적 및 프로젝트 패턴 파악
- **역방향 상태 의존성**: 이 기능이 변경·파괴할 상태(데이터/불변식)를 전제로 동작하는 기존 로직 탐색 — "내가 바꾼 코드가 무엇을 호출하는가"가 아니라 "내 기능이 만드는 상태 변화를 기존 코드 중 누가 가정하고 있는가"

각 에이전트에게 핵심 파일 5-10개 목록을 함께 반환하도록 요청합니다.
에이전트 결과를 받은 후, **해당 파일들을 직접 읽어 충분히 이해**한 뒤 탐색 결과를 요약합니다.

---

## STEP 3 — 질의응답

코드베이스 탐색 결과와 구현 요청을 검토하여 모호하거나 미확정된 사항을 정리합니다.
...
- **사용자의 답변을 기다린 후 STEP 4로 진행합니다**

사용자가 "알아서 해줘"라고 하면 권장안을 명시하고 명시적 확인을 받습니다.

---

## STEP 4 — 아키텍처 설계 (2-3개 `code-architect` 에이전트 병렬 실행, model: **opus**)

**반드시 2-3개의 `code-architect` 서브에이전트를 병렬로 호출합니다. `model: "opus"`를 명시합니다.**

각 에이전트는 다른 관점으로 설계안을 제시합니다:
- 최소 변경 (기존 코드 최대 재사용, 회귀 위험 최소화)
- 클린 아키텍처 (유지보수성, 우아한 추상화)
- 실용적 균형 (속도 + 품질)
- 운영 안정성: 대규모 벌크 처리 시 메모리(OOM) 방어, 이벤트 루프 블로킹 최소화, 캐싱 또는 큐 도입 필요성 검토

각 접근법의 트레이드오프를 비교하고, **권장안과 근거를 명시**합니다.
**사용자가 선호하는 방향을 선택할 때까지 기다립니다.**
...
## STEP 6 — 코드 리뷰 (`code-reviewer` + 언어별 리뷰어 에이전트 병렬 실행, model: **sonnet**)
...
## STEP 7 — 보안 리뷰 (`security-reviewer` 에이전트, model: **opus**)
...
## STEP 9 — 명세 문서 갱신 (`.claude/.spec/`)

**모든 구현 건은 이 단계를 거칩니다.** 명세 갱신 누락은 차후 회귀·온보딩·리뷰 비용을 키우므로 skip하지 않습니다. 단, 영향이 없으면 9.1에서 명시적으로 skip 보고합니다.
...
| 신규/변경 REST 엔드포인트 (행위는 동일, 경로·메서드·요청/응답 스키마 변경) | ✅ | ❌ | 해당 시 | ✅ | ✅ |
| 권한 키(`aw.menu.*`·`aw.org.admin` 등) 추가/매핑 변경 | ✅ | 해당 시 | ✅ | 해당 시 | 해당 시 |
| 환경변수 추가/기본값 변경/제거 | ✅ | ❌ | 해당 시 | ❌ | ❌ |
| 인증 모드·토큰 정책·rate limit·body limit 정책 변경 | ✅ | 해당 시 | ✅ | 해당 시 | 해당 시 |
| DB 스키마 변경 (엔티티 추가/컬럼 추가·삭제) | ✅ | ❌ | 해당 시 | ❌ | ❌ |
| 보안 정책 변경 (마스킹·`Cache-Control`·검증 규칙) | ✅ | ❌ | ✅ | 해당 시 | ❌ |
| 알고리즘/구현 결정 (PBKDF2 iteration, chunk 크기 등) | ✅ (1줄) | ❌ | ❌ | ❌ | ❌ |
| 내부 리팩토링·테스트 추가·주석·로깅·타입 정리 | ❌ | ❌ | ❌ | ❌ | ❌ |
| 단순 버그 수정 (외부 계약 불변) | 선택(눈에 띄는 사용자 영향이 있으면 ✅) | ❌ | ❌ | ❌ | ❌ |

표 결과 모든 칸이 ❌이면 STEP 9를 종료합니다 (보고에 `명세 영향 없음 — STEP 9 skip` 명시).

> **Test UI 시나리오 갱신은 STEP 9.9에서 별도 판단합니다.** 위 표와 독립적으로 적용됩니다.

### 9.2 `README.md` 변경 이력 추가
...
- **날짜**: 사용자 컨텍스트의 `currentDate`를 `YYYY-MM-DD`로 사용 (예: `2026-04-28`)
- **버전 식별자**: 기존 항목들에서 `1.0.x` 같은 패턴이 누적 중이면 다음 후보 번호를 제시하고 **사용자 확인을 받습니다.** 패턴이 명확하지 않거나 사용자가 식별자 부여를 원치 않으면 생략 가능
- **요약**: 1–3문장. 호환성 깸 여부·영향 받은 엔드포인트·새 환경변수·정책 강화 등 핵심만 (구현 디테일은 본문에 맡김)

### 9.3 `feature-list.md` 본문 갱신
...
### 9.3b `functional-spec.md` 본문 갱신

영향도 표에서 `functional-spec.md` ✅로 분류된 경우:

- 기존 섹션 번호(§2.x, §3.x 등) 체계를 따라 **신규 섹션 번호 부여** 또는 기존 섹션 갱신
- 시스템 동작 명세 (검증 규칙·권한 요건·모드별 차이·에러 코드·환경변수 의존성 포함)
- 모드별(USE_IUMS=true/false) 동작 차이 명시
- 권한 요건·rate limit·body limit이 있으면 함께 명시
- **안 들어가는 것**: private helper 이름, iteration 수·chunk 크기 같은 구현 상세 (dev_note에 위임)

### 9.4 `api.md` 본문 갱신

영향도 표에서 `api.md` ✅로 분류된 경우:

- 기존 섹션 번호 체계(§2.x, §5.x 등)를 따라 신규 섹션 추가 또는 기존 섹션 갱신
- 엔드포인트마다 다음 항목을 모두 포함:
  - HTTP 메서드 + 경로 + 인증 요건 + 권한 요건
  - 요청 쿼리/바디 스키마(필수/선택, 기본값, 검증 규칙)
  - 응답 스키마(성공·에러), envelope 사용 여부
  - rate limit·`Cache-Control` 등 정책
  - 호환성 깸 여부 명시
- list 엔드포인트는 `.claude/rules/rest-api.md` §1.4 체크리스트의 항목들이 모두 명세에 드러나야 함

### 9.5 Postman Collection 갱신
...
- `README.md`의 "환경변수 관련 주요 키" 표가 갱신되었는지 (신규 환경변수가 있다면)
- `features.md`와 `api.md`의 동일 기능 설명이 모순되지 않는지
- `.env.example`·`CLAUDE.md`·코드 내 상수 기본값과 명세의 기본값이 일치하는지
- 권한 키가 추가된 경우 `AwPermissions.ts`의 `AW_PERM`·`PERM_MAP`이 명세와 일치하는지
- `feature-list.md`의 항목 중 `functional-spec.md`에 대응 섹션이 없는 것이 있는지 (L1 ↔ L2 매핑 누락 검출)

### 9.7 보고 형식

명세 문서 변경 사항을 사용자에게 다음 형식으로 요약 보고:
...
- Test UI: <변경된 시나리오 목록> (또는 "영향 없음으로 skip")
```

영향 없음으로 분류된 경우: `명세 영향 없음 — STEP 9 skip`만 보고.

### 9.8 devnote 완료 기록

### 9.9 Test UI 시나리오 갱신

`packages/server/src/aw/test-ui/awTestUiHtml.ts`의 시나리오(S1–S10 및 IUMS 대응 버전)가 이번 구현으로 인해 검증 내용이 달라졌는지 판단합니다.

**갱신 트리거 (하나라도 해당하면 수행):**

| 변경 유형 | 갱신 대상 |
|---|---|
| 설정 카탈로그(`AW_CONFIG_DEFS`) 키 추가/제거 | S10 — 카탈로그 총 개수·키 목록 하드코딩 수정 |
| 감사 로그 action 값 추가/변경 | S9 — `assertField` 검증 action 문자열 수정 |
| 인증 플로우 변경 (상태 코드·헤더·응답 구조) | 해당 시나리오 |
| 새로운 권한 키 추가 | S7·S8 — 403 검증 대상 엔드포인트 또는 권한 키 수정 |
| CRUD 엔드포인트 경로·메서드 변경 | 해당 시나리오의 api 호출부 수정 |

**갱신이 필요 없는 경우 (명시적 skip 보고):**

- 내부 리팩토링, 타입 변경, 테스트 추가 등 외부 동작 불변
- 시나리오가 아직 커버하지 않는 기능 영역의 변경

**갱신 시 주의:**

- `awTestUiHtml.ts`는 TypeScript 템플릿 리터럴 내 inline JS이므로 `var` 사용, backtick·`${...}` 금지
- Standalone(`s1`~`s10`)과 IUMS(`s1I`~`s10I`)를 함께 수정합니다
- 하드코딩된 숫자(카탈로그 개수 등)는 코드 주석에 버전 기재 (예: `// 1.0.23: 10개`)
...
3. 없으면 파일 끝에 파일명을 한 줄 추가합니다 (예: `1.0.12_ver.md`)

이 단계는 명세 영향 없음 여부와 무관하게 항상 수행합니다.

---

위 요청을 바탕으로 STEP 1부터 시작합니다. **planner 에이전트를 즉시 호출합니다. `model: "opus"`를 명시합니다.**
